...
<p align="center">
  <img src="src/assets/logo.png" width="128" height="128" alt="老驴 Logo" />
</p>

<h1 align="center">老驴</h1>

<p align="center">
  <strong>老驴，把活跑起来</strong>
</p>

<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择老驴">为什么选择老驴</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**老驴** 是一款围绕 OpenClaw 打造的本地桌面工作台，把模型、渠道、技能和定时任务都收进同一个界面里，让你专心把活跑起来。

无论是自动化工作流、管理 AI 渠道，还是安排智能体定时执行任务，老驴都能把这些能力放到一套更直观、可控的桌面体验里。

老驴预置了常用模型提供商配置，支持 macOS、Windows、Linux 和多语言使用。需要更细的控制时，也可以在 **设置 → 高级 → 开发者模式** 里继续调整。
...
构建 AI 智能体不应该需要精通命令行。老驴 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
老驴 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，老驴 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
...
老驴 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。  
...
### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。老驴 自动适应你的偏好设置。

### 🚀 开机启动控制
在 **设置 → 通用** 中，你可以开启 **开机自动启动**，让 老驴 在系统登录后自动启动。
...
> Moonshot（Kimi）说明：老驴 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，老驴 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

老驴 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，老驴 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。
...
老驴 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 老驴 变得更好。
...
老驴基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
