...
<p align="center">
  <img src="src/assets/logo.png" width="128" height="128" alt="ステッド Logo" />
</p>

<h1 align="center">ステッド</h1>

<p align="center">
  <strong>ステッド、仕事を前に進める</strong>
</p>

<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜステッドなのか">なぜステッドなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**ステッド**は、OpenClaw をより扱いやすくするためのローカルデスクトップワークスペースです。モデル、チャンネル、スキル、定期実行タスクを 1 つの画面にまとめ、仕事を前に進めやすくします。

ワークフローの自動化、AI チャネルの管理、エージェントタスクのスケジューリングまで、ステッドは必要な操作を見通しよく一箇所に整理します。

ステッドは一般的なプロバイダー設定をあらかじめ備え、macOS、Windows、Linux と多言語に対応しています。必要に応じて **設定 → 詳細設定 → 開発者モード** から高度な挙動も調整できます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。ステッドはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
ステッドは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、ステッド はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
ステッド はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。  
...
### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。ステッドはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に ステッド が自動的に起動します。
...
ステッドを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：
...
ステッドには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、ステッドは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- ステッドはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
...
ステッドは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がステッドをより良くするのに役立ちます。
...
ステッドは優れたオープンソースプロジェクトの上に構築されています：
...
ステッドは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
