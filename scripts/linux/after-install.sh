...
# Post-installation script for 老驴 on Linux
...
# Create symbolic link for 老驴 app binary
if [ -x /opt/laolv/laolv ]; then
    ln -sf /opt/laolv/laolv /usr/local/bin/laolv 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/laolv/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/laolv/chrome-sandbox' || true
else
    chmod 0755 '/opt/laolv/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/laolv/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/laolv'
...
echo "老驴 has been installed successfully."
